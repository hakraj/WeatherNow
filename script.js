...
const WeatherIcon = (type) => {
  switch (type) {
    case "sunny":
      return `<svg class="w-8 h-8 text-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" role="img">
  <title>Sunny</title>
  <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
</svg>`;
    case "cloudy":
      return `<svg class="w-8 h-8 text-weather-cloud" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" role="img">
  <title>Cloudy</title>
  <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15a4.5 4.5 0 0 0 4.5 4.5H18a3.75 3.75 0 0 0 1.332-7.257 3 3 0 0 0-3.758-3.848 5.25 5.25 0 0 0-10.233 2.33A4.502 4.502 0 0 0 2.25 15Z" />
</svg>`;
    case "partly-cloudy":
            return `<div class="relative">
          <svg class="w-8 h-8 text-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" role="img">
        <title>Partly cloudy (sun)</title>
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
      </svg>
          <svg class="w-5 h-5 text-weather-cloud absolute -bottom-1 -right-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" role="img">
        <title>Partly cloudy (cloud)</title>
        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15a4.5 4.5 0 0 0 4.5 4.5H18a3.75 3.75 0 0 0 1.332-7.257 3 3 0 0 0-3.758-3.848 5.25 5.25 0 0 0-10.233 2.33A4.502 4.502 0 0 0 2.25 15Z" />
      </svg>
        </div>`;
    // case "rainy":
...
otherCitiesContainer.innerHTML = cities.map(city => `
    ...
    >
    <button class="delete-btn" onClick={deleteCard(this)} aria-label="Remove ${city.city} from cities">—</button>
      <div>
        ...
        card.addEventListener("click", () => {
          ...
          getCityCoordinates(cityName.toLowerCase()).then(place => {
            ...
            if (locationElement) {
              ...
              // Announce the location change to screen readers
              const announcement = document.createElement("div");
              announcement.setAttribute("role", "status");
              announcement.setAttribute("aria-live", "polite");
              announcement.className = "sr-only";
              announcement.textContent = `Now showing weather for ${place.name}, ${place.country}`;
...
function updateThemeIcon(isLight) {
  toggleBtn.innerHTML = isLight
    ? `<svg class="w-5 h-5 text-foreground" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" role="img">
